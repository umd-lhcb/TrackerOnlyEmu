 ...
 *
 * The particle is projected on the selected calorimeter surface, and the
 * projection information is retrieved. Tuple columns:
...
class TupleToolL0Calo : public TupleToolBase,
 ...
 public:
  ...
  TupleToolL0Calo( const std::string& type, const std::string& name,
                   const IInterface* parent );

  virtual ~TupleToolL0Calo(){};  ///< Destructor

  StatusCode initialize() override;
  StatusCode fill( const LHCb::Particle* /* mother */, const LHCb::Particle* P,
                   const std::string& head, Tuples::Tuple& tuple ) override;

 private:
  int isinside_HCAL( double x, double y );
  int isinside_ECAL( double x, double y );
