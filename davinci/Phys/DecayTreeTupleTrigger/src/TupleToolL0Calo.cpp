...
StatusCode TupleToolL0Calo::fill( const LHCb::Particle* /* mother */,
  ...
  double triggerET( -1. ), triggerHCALET( -1. ), xtrigger( 0. ),
      ytrigger( 0. );
  ...
  if ( m_calo == "HCAL" ) {
    ...
    test &=
        tuple->column( prefix + "_L0Calo_HCAL_TriggerHCALET", triggerHCALET );
...
int TupleToolL0Calo::isinside_HCAL( double x, double y ) {
  ...
  if ( TMath::Abs( x ) < HCAL_xMax_Outer &&
       TMath::Abs( y ) < HCAL_yMax_Outer ) {
    // projection inside inner calo (else is outer calo)
    if ( TMath::Abs( x ) < HCAL_xMax_Inner &&
         TMath::Abs( y ) < HCAL_yMax_Inner ) {
...
int TupleToolL0Calo::isinside_ECAL( double x, double y ) {
  ...
  // projection inside calo
  if ( TMath::Abs( x ) < ECAL_xMax_Outer &&
       TMath::Abs( y ) < ECAL_yMax_Outer ) {
    // projection inside middle calo (else is outer calo)
    if ( TMath::Abs( x ) < ECAL_xMax_Middle &&
         TMath::Abs( y ) < ECAL_yMax_Middle ) {
      // projection inside inner calo
      if ( TMath::Abs( x ) < ECAL_xMax_Inner &&
           TMath::Abs( y ) < ECAL_yMax_Inner ) {
        // projections outside the beampipe (in x)
...
//=============================================================================
double TupleToolL0Calo::getAssociatedCluster( double& hcal_energy,
                                              double& xTrigger,
                                              double& yTrigger ) {
  ...
  if ( m_part2calo->isValid() ) {
    ...
    BOOST_FOREACH ( LHCb::CaloCellID cell,
                    m_caloDe->neighborCells( centerCell ) ) {
  ...
  for ( cand = candidates->begin(); candidates->end() != cand; ++cand ) {
    ...
    if ( theCand->type() == typeToCheck ) {
      ...
      cell1 = theCand->id();
      cell2 = LHCb::CaloCellID( cell1.calo(), cell1.area(), cell1.row() + 1,
                                cell1.col() );
      cell3 = LHCb::CaloCellID( cell1.calo(), cell1.area(), cell1.row(),
                                cell1.col() + 1 );
      cell4 = LHCb::CaloCellID( cell1.calo(), cell1.area(), cell1.row() + 1,
                                cell1.col() + 1 );
      if ( std::binary_search( cells3x3.begin(), cells3x3.end(), cell1 ) ||
        ...
        if ( theCand->et() > result ) {
          ...
          if ( m_calo == "HCAL" ) {
            ...
            for ( std::vector<LHCb::L0CaloAdc>::const_iterator itAdc =
                      adcs.begin();
